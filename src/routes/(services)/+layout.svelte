...
<div class="h-screen flex flex-col">
    <div class="sticky top-0 bg-background z-50">
        <nav class="flex justify-between items-center mx-8 py-2 gap-8">
            <Button class="text-2xl font-bold" href="../" variant="link">TabularisRAG</Button>
            <div class="hidden md:flex justify-center items-center gap-2">
                <Button href="/documents" variant="link">{m['nav.items.documents']()}</Button>
                <Button href="/chat" variant="link">{m['nav.items.chat']()}</Button>
                <!--admin only-->
                <Button href="/users" variant="link">{m['nav.items.users']()}</Button>
                <Button href="/groups" variant="link">{m['nav.items.groups']()}</Button>
            </div>
            <div class="hidden md:flex justify-center items-center gap-2">
                <DropdownMenu.Root>
                    <DropdownMenu.Trigger>
                        {#snippet child({props})}
                            <Button {...props} variant="outline" class="h-9 w-9 lg:h-9 lg:px-4 lg:py-2 lg:w-fit">
                                <Globe/>
                                <span class="sr-only lg:not-sr-only">{m['nav.options.language']()}</span>
                            </Button>
                        {/snippet}
                    </DropdownMenu.Trigger>
                    <DropdownMenu.Content>
                        <DropdownMenu.Group>
                            <DropdownMenu.GroupHeading>{m['nav.options.language']()}</DropdownMenu.GroupHeading>
                ...
                <DropdownMenu.Root>
                    <DropdownMenu.Trigger>
                        {#snippet child({props})}
                            <Button {...props} variant="outline" class="h-9 w-9 lg:h-9 lg:px-4 lg:py-2 lg:w-fit">
                                <SunMoon/>
                                <span class="sr-only lg:not-sr-only">{m['nav.options.theme']()}</span>
                            </Button>
                        {/snippet}
                    </DropdownMenu.Trigger>
                    <DropdownMenu.Content>
                        <DropdownMenu.Group>
                            <DropdownMenu.GroupHeading>{m['nav.options.theme']()}</DropdownMenu.GroupHeading>
                            <DropdownMenu.Separator/>
                            <DropdownMenu.Group>
                ...
                <DropdownMenu.Root>
                    <DropdownMenu.Trigger>
                        {#snippet child({props})}
                            <Button {...props} variant="outline" class="h-9 w-9 lg:h-9 lg:px-4 lg:py-2 lg:w-fit">
                                <UserCog/>
                                <span class="sr-only lg:not-sr-only">{m['nav.items.profile']()}</span>
                            </Button>
                        {/snippet}
                    </DropdownMenu.Trigger>
            ...
            <DropdownMenu.Root>
                <DropdownMenu.Trigger>
                    {#snippet child({props})}
                        <Button {...props} variant="outline" size="icon" class="flex md:hidden">
                            <Menu/>
                        </Button>
                    {/snippet}
                </DropdownMenu.Trigger>
                    ...
                    <DropdownMenu.Group>
                        <DropdownMenu.Sub>
                            <DropdownMenu.SubTrigger>
                                <SunMoon class="mr-1 text-foreground"/>{m['nav.options.theme']()}
                            </DropdownMenu.SubTrigger>
                            <DropdownMenu.SubContent sideOffset={8}>
                                <DropdownMenu.Item onSelect={()=> setMode("dark")}>
                                    <Moon class="mr-1 text-foreground"/>{m['nav.options.dark']()}</DropdownMenu.Item>
                                <DropdownMenu.Item onSelect={()=> setMode("light")}>
                                    <Sun class="mr-1 text-foreground"/>{m['nav.options.light']()}</DropdownMenu.Item>
                                <DropdownMenu.Item onSelect={()=> resetMode()}>
                                    <MonitorSmartphone class="mr-1 text-foreground"/>{m['nav.options.system']()}</DropdownMenu.Item>
                            </DropdownMenu.SubContent>
                        </DropdownMenu.Sub>
                        <DropdownMenu.Sub>
                            <DropdownMenu.SubTrigger>
                                <Globe class="mr-1 text-foreground"/>{m['nav.options.language']()}
                            </DropdownMenu.SubTrigger>
                            <DropdownMenu.SubContent sideOffset={8}>
                                {#each languages as {code, name, flag}}
                                    <DropdownMenu.Item onSelect={()=>setLocale(code)}>
    ...
    <div class="grow flex-1 overflow-auto">
        {@render children()}
    </div>
</div>